...
Usage: {{ include "cost-onprem.initContainer.waitForKokuApi" . | nindent 8 }}
*/}}
{{- define "cost-onprem.initContainer.waitForKokuApi" -}}
- name: wait-for-koku-api
  image: "{{ .Values.global.initContainers.waitFor.repository }}:{{ .Values.global.initContainers.waitFor.tag }}"
  securityContext:
    {{- include "cost-onprem.securityContext.nonRoot" . | nindent 4 }}
  command: ['bash', '-c']
  args:
    - |
      echo "Waiting for Koku API at {{ include "cost-onprem.fullname" . }}-koku-api:8000..."
      until timeout 3 bash -c "echo > /dev/tcp/{{ include "cost-onprem.fullname" . }}-koku-api/8000" 2>/dev/null; do
