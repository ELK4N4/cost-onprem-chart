...
verify_recommendations() {
    ...
    local ready_url="http://localhost:32061/ready"
    local ready_response=$(curl -s -w "%{http_code}" --connect-timeout 5 --max-time 10 -o /dev/null "$ready_url" 2>/dev/null || echo "000")
    local ready_http_code="${ready_response: -3}"

    if [ "$ready_http_code" = "200" ]; then
        echo_success "✓ Ingress controller /ready endpoint accessible (HTTP $ready_http_code)"
    else
        echo_error "✗ Ingress controller /ready endpoint NOT accessible (HTTP $ready_http_code)"
    ...
    if false; then  # Disabled section - for reference
        ...
        echo_info ""
        echo_info "=== INGRESS CONTROLLER DIAGNOSTICS ==="
        echo_info "Checking ingress-nginx controller pods:"
        kubectl get pods -n ingress-nginx || true
        echo_info "Checking ingress-nginx controller service:"
        kubectl get service -n ingress-nginx ingress-nginx-controller -o wide || true
        echo_info "Checking ingress-nginx endpoints:"
        kubectl get endpoints -n ingress-nginx ingress-nginx-controller || true

        echo_info ""
        echo_info "=== CLUSTER RESOURCE STATUS ==="
        kubectl top nodes 2>/dev/null || echo "Metrics server not available"
        echo_info "All pods across cluster:"
        kubectl get pods --all-namespaces --field-selector=status.phase!=Running,status.phase!=Succeeded 2>/dev/null || echo "No non-running pods"
